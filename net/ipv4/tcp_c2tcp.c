...
int sysctl_tcp_c2tcp_enable __read_mostly = 1;
...
static void __maybe_unused c2tcp_reset(struct sock * sk)
{
   struct tcp_sock *tp = tcp_sk(sk);
   tp->c2tcp_min_urtt=0;
}
...
static void __maybe_unused init_deepcc(struct sock * sk)
{
   struct tcp_sock *tp = tcp_sk(sk);
...
static void c2tcp_pkts_acked(struct sock *sk, u32 cnt, s32 rtt_us)
{
    ...
    codel_time_t now;
    codel_time_t c2tcp_rtt;
    codel_time_t c2tcp_next_time;
    ...
    now = codel_get_time();
    c2tcp_rtt = MS2TIME(rtt_us / USEC_PER_MSEC);
    c2tcp_next_time = MS2TIME(tp->next_time / USEC_PER_MSEC);
...
static void bictcp_cwnd_event(struct sock *sk, enum tcp_ca_event event)
{
	if (event == CA_EVENT_TX_START) {
		 ...
		 * Shift epoch_start to keep cwnd growth to c2tcp curve.
		 */
...
/* calculate the c2tcp root of x using a table lookup followed by one
 * Newton-Raphson iteration.
 * Avg err ~= 0.195%
 */
static u32 c2tcp_root(u64 a)
{
	u32 x, b, shift;
	/*
...
static inline void bictcp_update(struct bictcp *ca, u32 cwnd, u32 acked)
{
	...
	/* The C2TCP function can update ca->cnt at most once per jiffy.
	...
	if (ca->epoch_start == 0) {
		...
		ca->tcp_cwnd = cwnd;			/* syn with c2tcp */
		...
		if (ca->last_max_cwnd <= cwnd) {
			 ...
			 */
			ca->bic_K = c2tcp_root(cube_factor
	...
	/* c2tcp function - calc bictcp_cnt*/
	if (bic_target > cwnd) {
	...
	/*
	 * The initial growth of c2tcp function may be too conservative
...
tcp_friendliness:
	...
	/* The maximum rate of cwnd increase C2TCP allows is 1 packet per
...
{
	struct tcp_sock __maybe_unused *tp = tcp_sk(sk);
	init_c2tcp(sk,val);
}
...
static int __init c2tcp_register(void)
{
	 ...
	 *  so K = c2tcp_root( (wmax-cwnd)*rtt/c )
